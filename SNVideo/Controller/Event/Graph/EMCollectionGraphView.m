...
-(void)setAxis
{
    plotSpace.yRange = [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat(_minCount) length:CPTDecimalFromFloat(_maxCount + 1)];
    NSString *yLength = [NSString stringWithFormat:@"%f", (_maxCount - _minCount) / 5];
    // 大刻度线间距： 50 单位
...
- (void)drawGraph
{
    ...
    //给画板添加一个主题
    CPTTheme *theme=[[ EMGraphTheme alloc ] init ];
   // CPTTheme *theme = [CPTTheme themeNamed:kCPTPlainWhiteTheme];
    ...
    x.majorIntervalLength = CPTDecimalFromFloat(24*60*60);//设置坐标轴的单位长度必须在x.labelTextStyle = titleStyle;（标注坐标点）这个之前否则坐标多会卡死
...
//    titleStyle.fontName = @"Helvetica-Bold";
    ...
    [x setAxisLineStyle:lineStyle_x];
   ...
   // x.majorGridLineStyle = lineStyle_x;
    ...
    // 大刻度线间距： 50 单位
    y. majorIntervalLength = CPTDecimalFromString ( yLength );
    y. majorTickLineStyle = lineStyle_x;
...
//    [self addLineWithIdentifier:@"Green Plot" color:[CPTColor colorWithComponentRed:63/255.0 green:180/255.0 blue:200/255.0 alpha:1]];
//    
//    [self addLineWithIdentifier:@"Green Plot2" color:[CPTColor colorWithComponentRed:252/255.0 green:210/255.0 blue:10/255.0 alpha:1]];
...
//    
    for (int i = 0; i < mar_identifiers.count; i++) {
        ...
        if ([plot.identifier isEqual:identifier]) {
            return [[_mdic_dataFprPlot valueForKey:identifier] count];
            break;
        }
    }
...
- (void)addLineWithIdentifier:(NSString *)identifier color:(CPTColor *)lineColor
{
   ...
   // lineStyle.lineColor = [CPTColor colorWithComponentRed:63/255.0 green:180/255.0 blue:200/255.0 alpha:1];
    ...
    plotSymbol.fill = [ CPTFill fillWithColor:[ CPTColor colorWithComponentRed:36/255.0 green:46/255.0 blue:60/255.0 alpha:1]];// 填充色：蓝色
